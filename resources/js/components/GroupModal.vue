                        ...
                        <div class="form-group">
                            ...
                            <ul>
                                <li v-for="user in form.users" :key="user.id">
                                    <user-name-data :user="user" />
                                    <span @click.prevent="clearStudent(user)">
                                        <svg width="20" height="20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.0.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M0 256C0 114.6 114.6 0 256 0C397.4 0 512 114.6 512 256C512 397.4 397.4 512 256 512C114.6 512 0 397.4 0 256zM168 232C154.7 232 144 242.7 144 256C144 269.3 154.7 280 168 280H344C357.3 280 368 269.3 368 256C368 242.7 357.3 232 344 232H168z"/></svg>
                                    </span>
                                </li>
                            </ul>
                            <label>Добавить студента в группу</label>
                            <user-roles
                                    :input_cut="false"
                                    @setCurrentUser="addUserToGroup"
                                >
                                <span slot="title" />
                                <template #table_actions_content>
                                    <span class="btn btn-primary">Добавить в группу</span>
                                </template>
                            </user-roles>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        addUserToGroup(user) {
            if(!this.form.users.find(groupUser => groupUser == user)) this.form.users.push(user)
        },
        setCurator(curator) {
        ...
        clearStudent(user) {
            let studentIndex = this.form.users.indexOf(user)
            this.form.users.splice(studentIndex, 1)
        }
    },
